...
RBP_graph.add((RB_DATA.Concordia_University, DBPEDIA_P.name, Literal("Concordia University")))
...
for idx, row in data.iterrows():
    ...
    course_with_code = URIRef(RB_DATA + f"{row["course_subject"].upper()}{row["course_number"]}")
    ...
    course_graph.add((course_with_code, DBPEDIA_P.number, Literal(row["course_number"])))
    course_graph.add((course_with_code, DBCORE.title,Literal(row["course_name"])))
    course_graph.add((course_with_code, RB_SCHEMA.numberOfCredits,Literal(int(row['credits']))))
    course_graph.add((course_with_code, SCHEMA.Event,Literal(row["course_component"])))
    course_graph.add((course_with_code, SCHEMA_THING.description,Literal(row["description"])))
    
    if c in courses_with_materials:
        if file_paths[c]['course_outline']:
            course_graph.add((course_with_code,RB_DATA.outline , URIRef(local_path+file_paths[c]['course_outline'][0])))
        else:
            course_graph.add((course_with_code,RB_DATA.outline , Literal("Please check UGrad/Grad Calendar")))
...
# adding Lectures as triples to a Lecture graph then to course graph
        ...
        for L in Lectures:
            ...
            if "slides" in L:
                Lecture_graph.add((Lecture_URI, RB_SCHEMA.slides, URIRef(local_path + L)))
            if "readings" in L:
                Lecture_graph.add((Lecture_URI, RB_SCHEMA.readings, URIRef(local_path + L)))
            if "worksheets" in L:
                Lecture_graph.add((Lecture_URI, RB_SCHEMA.worksheets, URIRef(local_path + L)))
                
            # adding Topics as triples to a topics graph

            topic_graph = Graph()
            for index, records in spotlight_annotations.iterrows():
                if L.split("/")[-1] == records["lecture_content"].split("/")[-1]:
                    topic_files_path = quote(local_path + records["lecture_content"], safe=":/")
                    ...
                    topic_graph.add((topicURI, DCTERMS.source, URIRef(topic_files_path)))
                    topic_graph.add((topicURI, RDFS.seeAlso, URIRef(records["topic_URI"])))
...
def create_student_graph(data, course_graph):
    ...
    for index, records in data.iterrows():
        ...
        if grade != "F":
            for _, _, lecture in course_graph.triples((course_URI, RB_SCHEMA.containsLecture, None)):
                for _, _, topic in course_graph.triples((lecture, RB_SCHEMA.coversTopic, None)):
...
RBP_graph.serialize("rbpgraph.ttl", format="ttl")
